...
def traverse_move(nodes, player, move):
    return [node.children[move] if p == player else node.children['_no_move'] for p, node in enumerate(nodes)]
...
def search_moismcts(initial_game_state, possible_hidden_states, num_iterations):
...
if __name__ == "__main__":
    ...
    action, roots = search_moismcts(start, possible_hidden_states, 100000)
    print action
    print {
        node.incoming_edge: (
            node.visit_count,
        ...
        for node in roots[0].children.values()
    }
